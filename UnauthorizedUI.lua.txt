...
local CONFIG = {
	...
	Durations = {
		...
		GlitchInterval = 1.1,    -- time between cycling error messages
		ButtonHover    = 0.16,
		ButtonPress    = 0.08,
		CopiedToast    = 1.6,
		TitlePulse     = 2.6,
...
local errorRed = create("TextLabel", {
	Size = UDim2.fromScale(1, 1),
	BackgroundTransparency = 1,
	Text = "",
	Font = Enum.Font.GothamBlack,
...
local errorBlue = create("TextLabel", {
	Size = UDim2.fromScale(1, 1),
	BackgroundTransparency = 1,
	Text = "",
	Font = Enum.Font.GothamBlack,
...
local function fragBurst()
	for i = 1, 10 do
		...
		local block = create("Frame", {
			...
			Position = UDim2.fromScale(rand(0.1, 0.9), rand(0.35, 0.6)),
			BackgroundColor3 = math.random() > 0.5 and CONFIG.Colors.Danger or CONFIG.Colors.Accent,
...
local function glitchSetText(newText)
	errorMain.Text = newText
	errorRed.Text = newText
	errorBlue.Text = newText
	fragBurst()

	for i = 1, 6 do
		errorRed.Position = UDim2.fromScale(0, 0) + UDim2.fromOffset(rand(-6, 6), rand(-3, 3))
		errorBlue.Position = UDim2.fromScale(0, 0) + UDim2.fromOffset(rand(-6, 6), rand(-3, 3))
		errorMain.Position = UDim2.fromScale(0, 0) + UDim2.fromOffset(rand(-2, 2), 0)
		task.wait(0.035)
	end
	tween(errorRed, ease(0.2, Enum.EasingStyle.Quad), { Position = UDim2.fromOffset(-3, 0) })
	tween(errorBlue, ease(0.2, Enum.EasingStyle.Quad), { Position = UDim2.fromOffset(3, 0) })
...
local function enterMain()
	hideAllScreens()
	mainScreen.Visible = true
	mainScreen.GroupTransparency = 0
end
...
local function enterError()
	...
	glitchSetText(CONFIG.ErrorMessages[1])

	task.spawn(function()
		local i = 1
		local elapsed = 0
		while errorScreen.Visible and elapsed < CONFIG.Durations.ErrorHold do
			task.wait(CONFIG.Durations.GlitchInterval)
			elapsed += CONFIG.Durations.GlitchInterval
			if not errorScreen.Visible then break end
			i = (i % #CONFIG.ErrorMessages) + 1
			glitchSetText(CONFIG.ErrorMessages[i])
		end
	end)
end
