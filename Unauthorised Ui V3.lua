...
local function bigButton(text, accentColor, layoutOrder)
	...
	-- small HUD diamond marker for a targeting-reticle feel
	create("Frame", {
...
----------------------------------------------------------------------
-- Backing glass panel (adds depth/layering behind the menu content)
----------------------------------------------------------------------
local menuPanel = create("Frame", {
	...
	stroke(CONFIG.Colors.AccentDim, 1, 0.45),
})
create("UIGradient", {
	Color = ColorSequence.new({
...
create("UIStroke", { Color = CONFIG.Colors.Accent, Thickness = 1, Transparency = 0.86, Parent = menuPanel })

-- HUD viewfinder-style corner brackets on the panel
local bracketBars = {}
local function cornerBracket(ax, ay)
	local h = create("Frame", {
		Size = UDim2.new(0, 26, 0, 2),
	...
	table.insert(bracketBars, h)
	table.insert(bracketBars, v)
end
cornerBracket(0, 0)
cornerBracket(1, 0)
cornerBracket(0, 1)
...
task.spawn(function()
	while menuPanel.Parent do
		for i, bar in ipairs(bracketBars) do
			task.delay((i - 1) * 0.08, function()
				if bar.Parent then
					tween(bar, ease(0.9, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.7 })
				end
			end)
		end
		task.wait(1.6)
		for i, bar in ipairs(bracketBars) do
			task.delay((i - 1) * 0.08, function()
				if bar.Parent then
					tween(bar, ease(0.9, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.15 })
				end
			end)
		end
		task.wait(1.6)
...
local badgeWrap = create("Frame", {
	...
	ZIndex = 2,
	Parent = mainScreen,
}, { corner(UDim.new(1, 0)), stroke(CONFIG.Colors.Danger, 1, 0.35) })
...
task.spawn(function()
	while badgeDot.Parent do
		tween(badgeDot, ease(0.8, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.75 })
		task.wait(0.8)
		tween(badgeDot, ease(0.8, Enum.EasingStyle.Sine), { BackgroundTransparency = 0 })
		task.wait(0.8)
	end
end)

-- slow rotating ring behind the title, for extra depth
local titleRing = create("Frame", {
	...
	ZIndex = 1,
	Parent = mainScreen,
}, { corner(UDim.new(1, 0)), stroke(CONFIG.Colors.AccentDim, 1, 0.87) })

task.spawn(function()
	while titleRing.Parent do
		tween(titleRing, ease(22, Enum.EasingStyle.Linear), { Rotation = titleRing.Rotation + 360 })
		task.wait(22)
	end
end)
...
local footerCursor = create("TextLabel", {
...
task.spawn(function()
	while footerCursor.Parent do
		tween(footerCursor, ease(0.5, Enum.EasingStyle.Quad), { TextTransparency = 1 })
		task.wait(0.5)
		tween(footerCursor, ease(0.5, Enum.EasingStyle.Quad), { TextTransparency = 0 })
		task.wait(0.5)
	end
end)

-- subtle mouse-follow parallax for a premium, layered feel
local menuPanelHome = menuPanel.Position
local titleWrapHome = titleWrap.Position
UserInputService.InputChanged:Connect(function(input)
	if input.UserInputType ~= Enum.UserInputType.MouseMovement then return end
	if not mainScreen.Visible then return end
	local camera = workspace.CurrentCamera
	local vp = camera and camera.ViewportSize or Vector2.new(1920, 1080)
	local nx = math.clamp((input.Position.X / vp.X - 0.5) * 2, -1, 1)
	local ny = math.clamp((input.Position.Y / vp.Y - 0.5) * 2, -1, 1)
	tween(menuPanel, ease(0.5, Enum.EasingStyle.Sine), {
		Position = menuPanelHome + UDim2.fromScale(nx * 0.01, ny * 0.008),
	})
	tween(titleWrap, ease(0.5, Enum.EasingStyle.Sine), {
		Position = titleWrapHome + UDim2.fromScale(nx * 0.016, ny * 0.012),
	})
end)
...
local crossGlow1 = crossBar(45, true)
local crossGlow2 = crossBar(-45, true)
-- crisp bars on top
local crossFront1 = crossBar(45, false)
local crossFront2 = crossBar(-45, false)
local crossStroke1 = create("UIStroke", { Color = CONFIG.Colors.White, Thickness = 1, Transparency = 0.4, Parent = crossFront1 })
local crossStroke2 = create("UIStroke", { Color = CONFIG.Colors.White, Thickness = 1, Transparency = 0.4, Parent = crossFront2 })
...
-- gentle side-to-side sway (kept slow and small so it reads as "alive", not "shaking")
task.spawn(function()
	while crossHolder.Parent do
		if errorActive then
			tween(crossHolder, ease(2.2, Enum.EasingStyle.Sine), { Rotation = 5 })
			task.wait(2.2)
			tween(crossHolder, ease(2.2, Enum.EasingStyle.Sine), { Rotation = -5 })
			task.wait(2.2)
		else
...
-- soft glow-bar brightness pulse behind the bars
task.spawn(function()
	while crossGlow1.Parent do
		if errorActive then
			tween(crossGlow1, ease(1, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.55 })
			tween(crossGlow2, ease(1, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.55 })
			task.wait(1)
			tween(crossGlow1, ease(1, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.8 })
			tween(crossGlow2, ease(1, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.8 })
			task.wait(1)
		else
			task.wait(0.3)
		end
...
-- crisp-bar outline shimmer
task.spawn(function()
	while crossStroke1.Parent do
		if errorActive then
			tween(crossStroke1, ease(0.9, Enum.EasingStyle.Sine), { Transparency = 0.1 })
			tween(crossStroke2, ease(0.9, Enum.EasingStyle.Sine), { Transparency = 0.1 })
			task.wait(0.9)
			tween(crossStroke1, ease(0.9, Enum.EasingStyle.Sine), { Transparency = 0.55 })
			tween(crossStroke2, ease(0.9, Enum.EasingStyle.Sine), { Transparency = 0.55 })
			task.wait(0.9)
		else
			task.wait(0.3)
		end
...
----------------------------------------------------------------------
-- FRIENDLY "SIREN" LIGHTS — soft alternating red/blue beacon glow
-- Deliberately slow and soft (not a fast strobe) so it reads as
-- playful/alert rather than scary.
----------------------------------------------------------------------
local CONFIG_SirenBlue = Color3.fromRGB(90, 160, 255)

local sirenLayer = create("Frame", {
	Size = UDim2.fromScale(1, 1),
	BackgroundTransparency = 1,
	ZIndex = 2,
	Parent = errorScreen,
...
local function beaconGlow(color, posX)
	return create("Frame", {
		Size = UDim2.fromOffset(460, 460),
		AnchorPoint = Vector2.new(0.5, 0.5),
		Position = UDim2.fromScale(posX, -0.08),
...
local function beaconLamp(color, posX)
	return create("Frame", {
		Size = UDim2.fromOffset(20, 20),
		AnchorPoint = Vector2.new(0.5, 0.5),
		Position = UDim2.fromScale(posX, 0.05),
		BackgroundColor3 = color,
		BackgroundTransparency = 1,
		BorderSizePixel = 0,
		ZIndex = 3,
		Parent = sirenLayer,
	}, { corner(UDim.new(1, 0)), stroke(CONFIG.Colors.White, 1, 0.5) })
end

local sirenRedGlow = beaconGlow(CONFIG.Colors.Danger, 0.12)
local sirenBlueGlow = beaconGlow(CONFIG_SirenBlue, 0.88)
local sirenRedLamp = beaconLamp(CONFIG.Colors.Danger, 0.12)
local sirenBlueLamp = beaconLamp(CONFIG_SirenBlue, 0.88)

task.spawn(function()
	while sirenLayer.Parent do
		if errorActive then
			tween(sirenRedGlow, ease(0.75, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.8 })
			tween(sirenBlueGlow, ease(0.75, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.96 })
			tween(sirenRedLamp, ease(0.75, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.1 })
			tween(sirenBlueLamp, ease(0.75, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.75 })
			task.wait(0.75)
			tween(sirenRedGlow, ease(0.75, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.96 })
			tween(sirenBlueGlow, ease(0.75, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.8 })
			tween(sirenRedLamp, ease(0.75, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.75 })
			tween(sirenBlueLamp, ease(0.75, Enum.EasingStyle.Sine), { BackgroundTransparency = 0.1 })
			task.wait(0.75)
		else
			tween(sirenRedGlow, ease(0.4), { BackgroundTransparency = 1 })
			tween(sirenBlueGlow, ease(0.4), { BackgroundTransparency = 1 })
			tween(sirenRedLamp, ease(0.4), { BackgroundTransparency = 1 })
			tween(sirenBlueLamp, ease(0.4), { BackgroundTransparency = 1 })
			task.wait(0.3)
		end
	end
end)
